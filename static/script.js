document.addEventListener('DOMContentLoaded', () => {
    ...
    // Updated event listeners for speakRecogBtn and speakTransBtn
    speakRecogBtn.addEventListener('click', () => {
        speakText1(recognizedTextOutput.value);
    });
    
    speakTransBtn.addEventListener('click', () => {
        const language = languageSelect.value;
        if (language=="en"){
            speakText1(translatedTextOutput.value);
        }
        else{
            speakText(translatedTextOutput.value, language);
        }
    });
